...
def app(event, context):
   ...
   #soup_1 = BeautifulSoup(page.text, 'html.parser')
        directory_periodic = "/news/final/periodico=bbc/year="+year+"/month="+month+"/day="+day
        #directory_search_bbc = "/headlines/raw/Periodico=bbc/year="+year+"/month="+month+"/day="+day+"/bbc.html"
        s3 = boto3.resource('s3')
        obj = s3.Object('bcc2','headlines/raw/Periodico=bbc/year=bbc/month=bbc/day=bbc/bbc.html')
       # j=obj.get()['Body'].read().decode('utf-8')
        ...
        s3Object_1 = s3.Object('bucketbcc2',directory_periodic+'/bbc.csv')
        s3Object_1.put(
                Body=csv
        )